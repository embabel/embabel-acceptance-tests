...
class InterestingFactsAgentTest {

    @Test
    @DisplayName("Should find interesting facts about a person")
    void shouldFindInterestingFacts(ServerInfo server) throws IOException {
        String baseUrl = server.getBaseUrl();
        String payload = loadJsonPayload("payloads/interesting-facts-request.json");
        
        System.out.println("Requesting interesting facts at: " + baseUrl);
        ...
        assertThat(statusCode)
            .as("Server should accept the request to find interesting facts")
            .isIn(200, 202);
        
        if (statusCode == 200) {
            response.then()
                .body("jsonrpc", equalTo("2.0"))
                .body("id", equalTo("req-006"))
                .body("result", notNullValue());
            
            System.out.println("✓ Interesting facts found successfully");
        } else {
            System.out.println("✓ Interesting facts request accepted for async processing");
        }
    }
    ...
    @Test
    @DisplayName("Should verify message structure with person details")
    void shouldVerifyMessageStructure(ServerInfo server) throws IOException {
        String baseUrl = server.getBaseUrl();
        String payload = loadJsonPayload("payloads/interesting-facts-request.json");
        
        assertThat(payload)
            .as("Payload should contain person details")
            .contains("\"kind\": \"text\"")
